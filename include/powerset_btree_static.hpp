#ifndef EFFICIENT_DST_POWERSET_BTREE_STATIC_HPP
#define EFFICIENT_DST_POWERSET_BTREE_STATIC_HPP
...
namespace efficient_DST{
	...
	public:
		...
		powerset_btree_static(FOD* _fod, const size_t _block_size) :
			node_pool(_block_size),
			root(create_disjunction_node(nullptr, nullptr, nullptr)),
			number_of_non_null_values(0),
			fod(_fod),
			block_size(_block_size)
		{}

		powerset_btree_static(const powerset_btree_static<T>& p) :
			powerset_btree_static(p.fod, p.block_size)
		{
			copy(p);
		}

		// constructor for default initialization (required in unordered_map)
		powerset_btree_static() :
			node_pool(0),
			root(nullptr),
			number_of_non_null_values(0),
			fod(),
		...
		void init_tree(){
			this->root = create_disjunction_node(nullptr, nullptr, nullptr);
		}

		void copy(const powerset_btree_static<T>& p){
		...
		void copy_sets(const powerset_btree_static<T>& p, T default_value){
		...
		set_N_value<T>* insert(const boost::dynamic_bitset<>& set, const T& value){
			...
			size_t d = 0;
			for(; d < this->fod->size()-1; ++d){
				if(set[d]){
					if(!target->right){
						target->right = create_disjunction_node(target, nullptr, nullptr);
					}
					target = target->right;
				}else{
					if(!target->left){
			...
			if(set[d]){
				if(!target->right){
					target->right = create_node(set, value, target, nullptr, nullptr);
				}else{
		...
		set_N_value<T>* find(const boost::dynamic_bitset<>& set) const {
			node<T>* target = this->root;
			for(size_t d = 0; d < this->fod->size(); ++d){
				if(set[d]){
					target = target->right;
				}else{
					target = target->left;
...
/*
		void fill_with_union_of_powersets(
				const powerset_btree_static<T>& powerset1,
				const powerset_btree_static<T>& powerset2,
...
/*
		std::vector<set_N_value<T>* > subsets_of(const boost::dynamic_bitset<>& set, const bool& include_disjunction_nodes) const {
			return subsets_of(set, false, include_disjunction_nodes);
		}
*/
...
*/
		std::vector<set_N_value<T>* > strict_subsets_of(const std::vector<fod_element*>& fod_elements) const {
			return subsets_of(this->fod->to_set(fod_elements), true);
		}
		...
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > strict_subsets_of_by_cardinality(const boost::dynamic_bitset<>& set) const {
			return subsets_of_by_cardinality(set, true);
		}
/*
...
*/
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > strict_subsets_of_by_cardinality(const std::vector<fod_element*>& fod_elements) const {
			return subsets_of_by_cardinality(this->fod->to_set(fod_elements), true);
		}
...
*/
		...
		std::vector<set_N_value<T>* > strict_supersets_of(const boost::dynamic_bitset<>& set) const {
			return supersets_of(set, true);
		}
/*
		...
		std::vector<set_N_value<T>* > strict_supersets_of(const std::vector<fod_element*>& fod_elements) const {
			return supersets_of(this->fod->to_set(fod_elements), true);
		}
		...
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > strict_supersets_of_by_cardinality(const boost::dynamic_bitset<>& set) const {
			return supersets_of_by_cardinality(set, true);
		}
/*
...
*/
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > strict_supersets_of_by_cardinality(const std::vector<fod_element*>& fod_elements) const {
			return supersets_of_by_cardinality(this->fod->to_set(fod_elements), true);
		}
		...
		static const std::vector<boost::dynamic_bitset<> > unions_with_not_subsets_of_smaller_than(
				const powerset_btree_static<T>& powerset,
		...
		static void flip_powerset_from_to(const powerset_btree_static<T>& powerset1, powerset_btree_static<T>& powerset2){
		...
		static void flip_powerset_from_to(const powerset_btree_static<T>& powerset1, powerset_btree_static<set_N_value<T>* >& powerset2){
			const std::vector<set_N_value<T>* >& powerset1_list = powerset1.elements();
	...
	protected:
		...
		node<T>* create_node(const boost::dynamic_bitset<>& set, T value, node<T>* parent_node, node<T>* left_node, node<T>* right_node){
			node<T>* new_node = this->node_pool.emplace(set, value, parent_node, left_node, right_node);
		...
		node<T>* create_disjunction_node(node<T>* parent_node, node<T>* left_node, node<T>* right_node){
			node<T>* new_node = this->node_pool.emplace(parent_node, left_node, right_node);
			if(left_node)
				new_node->left->parent = new_node;
			if(right_node)
				new_node->right->parent = new_node;
...
*/
		...
		static void elements_by_cardinality(node<T>* leaf, std::unordered_map<size_t, std::vector<set_N_value<T>* > >& all_values) {
			if(leaf->left){
				if(leaf->right){
		...
		static void elements(node<T>* leaf, std::vector<set_N_value<T>* >& all_values) {
			if(leaf->left){
				if(leaf->right){
					elements(leaf->left, all_values);
				}
		...
		std::vector<set_N_value<T>* > subsets_of(const boost::dynamic_bitset<>& set, const bool& strict) const {
		...
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > subsets_of_by_cardinality(
				const boost::dynamic_bitset<>& set, const bool& strict) const {
		...
		std::vector<set_N_value<T>* > supersets_of(const boost::dynamic_bitset<>& set, const bool& strict) const {
		...
		std::unordered_map<size_t, std::vector<set_N_value<T>* > > supersets_of_by_cardinality(const boost::dynamic_bitset<>& set, const bool& strict) const {
...
#endif // EFFICIENT_DST_POWERSET_BTREE_STATIC_HPP
